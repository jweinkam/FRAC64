             ...
             png_charp profile, png_uint_32 proflen)
{
   ...
   namelen = png_strlen(name)+1;
   new_iccp_name = (png_charp)png_malloc(png_ptr, (png_uint_32)namelen);
   png_strcpy(new_iccp_name, namelen, name);
             ...
             png_infop info_ptr, png_sPLT_tp entries, int nentries)
{
    ...
    for (i = 0; i < nentries; i++)
    {
		...
		namelen = png_strlen(from->name) + 1;
        to->name = (png_charp)png_malloc(png_ptr,
            (png_uint_32)namelen);
        png_strcpy(to->name, namelen, from->name);
   ...
   png_infop info_ptr, png_unknown_chunkp unknowns, int num_unknowns)
{
    ...
    for (i = 0; i < num_unknowns; i++)
    {
        ...
        png_strcpy((png_charp)to->name, 5, (png_charp)from->name);
